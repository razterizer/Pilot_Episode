...
void draw_sky(SpriteHandler<NR, NC>& sh)
{
  ...
  {
    sh.replace_bg_color(Text::Color::Transparent, Text::Color::DarkRed, sky_bb);
    sh.replace_bg_color(Text::Color::Transparent2, Text::Color::DarkRed, sky_bb);
  }
  else if (plane_data::blackout_state == plane_data::BlackoutState::Stall)
  {
    sh.replace_bg_color(Text::Color::Transparent, Text::Color::DarkYellow, sky_bb);
    sh.replace_bg_color(Text::Color::Transparent2, Text::Color::DarkYellow, sky_bb);
  }
  else if (plane_data::blackout_state == plane_data::BlackoutState::Blackout)
  {
    sh.replace_fg_color(Text::Color::Black, sky_bb);
    sh.replace_bg_color(Text::Color::Black, sky_bb);
  }
  else if (plane_data::alt_ft > alt_soft_limit_ft) // A cessna can fly up to a height of 51'000 ft.
  {
    ...
    sh.replace_bg_color(Text::Color::Transparent, Text::Color::DarkBlue, { 1, 1, h_offs, 77 });
    sh.replace_bg_color(Text::Color::Transparent2, Text::Color::DarkBlue, { 1, 1, h_offs, 77 });
    if (plane_data::alt_km >= 100) // Kármán line.
    {
      auto t = (plane_data::alt_km - 100)/(400 - 100);
      if (t > 1) t = 1;
      int h_offs = math::roundI(27*t);
      sh.replace_bg_color(Text::Color::DarkBlue, Text::Color::Black, { 1, 1, h_offs, 77 });
    }
  }
}
