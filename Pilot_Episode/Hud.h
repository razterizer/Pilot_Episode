              ...
              int score)
{
  ...
  std::string life_str = "H: ["
    ...
    + "]";
  sh.write_buffer(life_str, 0, hud_c, Color16::Cyan);
  //sh.write_buffer(std::to_string(health), 5, 5, Color16::Black);
  hud_c += 1 + static_cast<int>(life_str.size());
  std::string alt_str = "Alt: ";
  plane_data::alt_px = ground_level - plane_data::y_pos + 13*t8x::pix_ar2;
  ...
  alt_str += " ft,";
  sh.write_buffer(alt_str, 0, hud_c, Color16::Cyan);
  ...
  gnd_speed_str += " kn,";
  sh.write_buffer(gnd_speed_str, 0, hud_c, Color16::Cyan);
